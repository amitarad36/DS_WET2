...
public:
	...
	StatusType resize(bool to_enlarge) {
		bool execute = false;
		if (to_enlarge) {
			if (getLoadFactor() >= UPPER_LOAD_FACTOR_LIMIT) execute = true;
		}
		else {
			if (getLoadFactor() <= LOWER_LOAD_FACTOR_LIMIT) execute = true;
		}

		if (execute) {
			int new_hash_table_size = m_hash_table_size / 2;
			if (to_enlarge) new_hash_table_size = m_hash_table_size * 2;
			// constructing the new array
			BinaryTree<T>** new_data = new BinaryTree<T>*[new_hash_table_size];
			...
			delete m_data;
			m_data = new_data;
			m_hash_table_size = new_hash_table_size;
		}
	}
};